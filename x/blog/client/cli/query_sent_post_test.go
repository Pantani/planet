...
func (s *IntegrationTestSuite) networkWithSentPostObjects(n int) (*network.Network, []types.SentPost) {
	s.T().Helper()
	...
	return s.network(&state), state.SentPostList
}

func (s *IntegrationTestSuite) TestShowSentPost() {
	var (
		net, objs = s.networkWithSentPostObjects(2)
...
func (s *IntegrationTestSuite) TestListSentPost() {
	var (
		net, objs = s.networkWithSentPostObjects(5)
		ctx       = net.Validators[0].ClientCtx
	)
